...
local({
  files <- list()
  lines <- baseline
  lines <- remove_block(lines, "## Materials", "  - `example-tool` (https://example.org/example-tool) — version 1.0.", remove_start = TRUE)
  files[["example-protocol"]] <- lines
...
local({
  files <- list()
  lines <- baseline
  lines <- remove_block(lines, "## History & Reviews", "- **Notes:** Read for scientific soundness. The parameters in Step 1 are appropriate.", remove_start = TRUE)
