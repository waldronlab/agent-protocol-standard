...
Two further problems were only visible once the corpus and the backlog were looked at together.
...
Requiring the easy question also removes pressure to misfile the hard one. The standing citation policy
exists because authors cite the paper they read where the method's origin belongs. Giving the read paper a
correct home of its own does more than a rule forbidding the substitution.
